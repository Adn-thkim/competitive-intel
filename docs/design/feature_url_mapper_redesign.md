...
| **D42** *(turn-60 신설, [x] 확정)* | v0.10.27 와 v0.10.25 의 진행 순서 | **(a) v0.10.27 → v0.10.25 + 임시 `_union_raw_features` 헬퍼 신설** | (a) **v0.10.27 PR 내 임시 헬퍼 (priority 기반 dedup union) — v0.10.25 정식화 시 candidate_coverage union 으로 교체 (채택)** — 본 PR 단독 검증 가능, (b) 단일 PR 통합 — 약 +600줄 review 부담, (c) v0.10.25 선행 + 5종 호환 어댑터 → v0.10.27 — 시리즈 의존 순서 역전 |
