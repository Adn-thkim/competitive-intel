...
| **v0.10.23** (P2 분배 실 구현, turn-56)              | `agents/feature_mapping_<source>/system_prompt_kr.md` 5종 + `output.schema.json` 5종 신설 (D39 b·D40 a) + 기존 `agents/feature_url_mapper/output.schema.json` 의 origin enum 6종 갱신 (D41 b — 후방 호환). 5 prompt 각자 담당 report_type 1~3종 정책 + source 특수 메타 활용 (subpage_category·domain_class·view/like/comment_count·platform/account_scope·source_tier/tier_group)                                                                                                                                                                                                                                                                                                                                                                                                                                                |         약 +1,100줄 (5 prompt × 110 + 5 schema × 100 + 기존 schema +1)          | 5 prompt 모두 jsonschema validate 통과 + 5 schema 의 origin enum 6종 정확 정의 + 5 prompt 각자 담당 report_type 만 처리 + 공통 base 정책 (feat_·coverage·priority·JSON only) 5 prompt 일관                                                                                                                                                    |
| **v0.10.24** (P3 실 구현, turn-66)              | `_MetaExtractor` 보강 (`<h1>`/`<h2>` + 본문 첫 800자 + `<meta article:published_time>`/`<time datetime>` 발행일 추출) + `_fetch_meta` cache_context `v=1→v=2` bump (자동 무효화) + `_build_candidates_with_meta` 신규 메타 carry + `_is_recent_enough(max_months=36)` 헬퍼 + `_feature_mapping_runner` blog_community 분기 발행일 검증 (D37 적용) + 3종 system_prompt (`official`·`blog_community`·`macro`) 의 입력 구조/판정 기준에 `h1_h2`·`body_excerpt` 활용 정책 추가                                                                                                                                                                                                                                                                                                                                                            |                                   약 +250줄                                   | 정적 사이트 5개에서 body_excerpt 추출 성공률 100% + blog_community 의 36개월 초과 후기 자동 제외 + comparison_matrix coverage=sufficient 비율 30% → 50% 이상 + D37 부채 해소                                                                                                                                                                            |
| **v0.10.25** (P3 실 구현, turn-67)              | `additional_urls_validation_node` 정식화 — (1) D23 정식 `_union_raw_features` (v0.10.27.1 임시 헬퍼 폐기 → 본 모듈 내부 정식 이전 + `source_origin` 메타 부착) + (2) 5 source 별 검증 분기 (`_validate_youtube_video` videos.list / `_validate_owned_channel` is_brand_match / `_validate_official_subpage` HEAD/GET / `_validate_blog_community` HEAD/GET + 36개월 / `_validate_macro_subpage` HEAD/GET + 화이트리스트) + (3) 임시 fallback (state["raw_features"]) 폐기 + (4) youtube_videos_list public 함수 신설 (1 unit/call) + (5) 5종 system_prompt 의 additional_urls `source_origin` 필드 명시                                                                                                                                                                                                                                                            |                                   약 +500/-130줄 (순증가 +370)                                   | YouTube `additional_urls` `view_count`·like·comment 채움 + 삭제·private 영상 validated=False + owned_channel `is_brand_match` 채움 + battlecard feature_id 의 candidate_coverage union 정상 + blog_community additional_urls 발행일 ≤ 36개월 + macro additional_urls 화이트리스트 매칭                                                                                                                                                                                      |
| **v0.10.26** *(P1c-1 실 구현, turn-58)*            | `cross_reference_node` 신설 — youtube_reactions × owned_channels(youtube_official) 결정론적 후처리 필터링. `page_meta_collect_node` 폐기는 v0.10.27 통합 노드 도입 시점으로 이연 (본 PR 에서는 `urls_merge → page_meta_collect` 흐름 유지). graph.py 토폴로지: 5중 list-fan-in target 을 `urls_merge` → `cross_reference` 로 변경 + `cross_reference → urls_merge` edge 신설                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |                                   약 +130줄                                   | mock owned channel ID 주입 시 reactions 결과에서 0건 잔존 + 다른 4종 키 변경 없음 + graph.py 검증 출력에 cross_reference 노드 + 5 fan-in edge 가 cross_reference 로 변경됨 확인 + wall-clock < 50ms                                                                                                                                                    |
| **v0.10.27** *(P1c-2 실 구현, turn-60)*            | `feature_mapping_<source>_node` 5종 신설 + `_feature_mapping_runner.run_source_mapping` 공통 헬퍼 + 단일 `urls_merge_node`·`page_meta_collect_node`·`feature_mapping_llm_node` 3 파일 삭제 + `agents/feature_url_mapper/` 디렉토리 삭제 (D43). 5중 fan-out (2차) 토폴로지 활성화. `additional_urls_validation_node` 에 임시 `_union_raw_features` 호환 어댑터 (D42 a — v0.10.25 정식화 시 폐기). state.py 에 5종 `*_raw_features` 키 신설. graph.py 검증 변수 `_fanout_5_1`·`_fanin_5_1`·`_fanout_5_2`·`_fanin_5_2` 4종 + v0.10.27 표기                                                                                                                                                                                                                                                                                                                      |                          약 +850 / -680줄 (순증가 +170)                          | 5 통합 노드 각자 `*_raw_features` 산출, LLM 호출 수 8회(parallel=4), wall-clock 약 17분(cache miss) + graph.py 검증 출력 v0.10.27 토폴로지 4중 fan + 1 직렬 정상                                                                                                                                                                                  |
...
|                ID                 | 항목                                                        | 권장안                                                                                                                                                                        | 트레이드오프                                                                                                                                                                                                                                                                              |
| :-------------------------------: | --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **D14** *(turn-7 신설, [x] 확정, v0.10.21 적용)* | X (트위터) 처리 정책                                             | **(b) 1차 핸들 발견까지만 + 본문 metadata 미수집** — `url_discovery_owned_channels_node` 의 platform 처리에서 핸들 식별까지만 책임. 본문 metadata 수집은 v1.0 §6-6a `pr_release_collection_node` 후속 위임 | (a) 1차 제외 — D11 앱스토어 사상, (b) **URL 발견만 (채택)**, (c) playwright 본문 fetch                                                                                                                                                                                                                       |
| **D15** *(turn-7 신설, [x] 확정, v0.10.20 적용)* | YouTube API quota 한도 도달 시 동작                              | **(b) `agent_steps[*].status="quota_skip"` 우회 + 부분 결과로 진행** — `youtube_client.py` 의 quota guard + `url_discovery_youtube_reactions_node` graceful 처리                                                                                                                        | (a) 한도 도달 시 전체 노드 fail, (b) **부분 결과로 진행 (채택)**, (c) Brave 검색 fallback                                                                                                                                                                                                               |
| **D16** *(turn-7 신설, [x] 확정, v0.10.21 적용)* | Owned channels LLM 검증 confidence 임계                       | **(b) 단일 임계 0.7** — `_LLM_CONFIDENCE_THRESHOLD = 0.7` 모듈 상수 + 0.7 미만 항목 자동 제외                                                                                                                                   | (a) 자동 채택 0.5 / 자동 거부 0.3 / 사이는 사용자, (b) **단일 임계 0.7 (채택)**, (c) 모두 사용자 검토                                                                                                                                                                                                                   |
| **D17** *(turn-7 신설, [x] 확정, v0.10.21 적용)* | 다중 공식 계정 발견 시 처리                                          | **(b) 모든 발견 핸들 반환 + `account_scope` 4종 분류** — parent_company·sub_brand·product_specific·regional. v0.10.28b 의 OwnedChannelCard UI 에서 candidate × platform 매트릭스 + scope chip 표시                                                                                                                                                                                                                                                                                                                       | (a) primary 자동 선택, (b) **모두 반환 + UI 토글 (채택)**, (c) 사용자 매번 선택                                                                                                                                                                                                                                 |
...
| **D19** *(turn-3 신설, [x] 확정, v0.11 위임)* | playwright 트리거 조건                                         | **(a) 도메인 화이트리스트 + `<title>` 부재 + HTML 길이 < 1KB 결합 채택** — 본 시리즈 (v0.10.x) 에서는 명세 확정, 실 구현은 v0.11 (playwright SPA fallback) PR 에서 도입                                                                                                                                  | (a) **도메인 화이트리스트 + `<title>` 부재 + HTML 길이 < 1KB (채택)**, (b) HTML heuristic 만, (c) 모든 비-OK 페이지                                                                                                                                                                                                                              |
| **D20** *(turn-3 신설, [x] 확정, v0.10.22 적용)* | `market_context_swot` 의 A 부분 처리                           | **(b) `url_discovery_macro_node` 1차 처리 + v1.0 §6-6a 인계** — v0.10.22 에서 2-layer 화이트리스트 + Tier 그룹 site: + 2단계 fallback + candidate_id="macro" 단일 키 적용. v1.0 에서 `market_context_collection_node` 가 본문 통계 추출 인계                                                                                  | (a) v1.0 까지 미처리(빈 결과), (b) **`url_discovery_macro_node` 1차 처리 (채택)**                                                                                                                                                                                                                         |
...
| **D22** *(turn-7 신설, [x] 확정, v0.10.26 적용)* | cross-reference 신선도 보장 방식                                 | **(d) `cross_reference_node` 별도 노드 분리로 단순화 (turn-11 옵션 (e) 채택의 부수 효과)** — 동일 graph 실행 내 1회 발화로 `prompt_version` bump + `updated_at` 비교 모두 불필요. v0.10.26 의 cross_reference_node 는 캐시 미사용 + 결정론적 결과 + wall-clock < 50ms                                           | (a) 두 노드 캐시 TTL 합산 최소값 강제, (b) `prompt_version` 동시 bump, (c) `updated_at` 비교, (d) **별도 노드 분리로 단순화 (채택)** — turn-11 옵션 (e) 의 부수 효과                                                                                                                                                                                      |
|      **D23** *(turn-11 신설, [x] 확정, turn-67 적용)*       | `battlecard` 등 A+B 흐름 feature 의 다중 source 매핑 통합 정책        | **(a) 정식 적용** — `additional_urls_validation_node._union_raw_features` (v0.10.25) 가 candidate_coverage union 처리. v0.10.27.1 의 임시 헬퍼 (feature_url_mapper_node.py 잔존) 폐기. existing_urls·additional_urls URL union (dedup) + coverage 갱신 + **`source_origin` 메타 부착** (`additional_urls` 검증 분기 키).                                                                    | (a) **union 처리 + source_origin 메타 부착 (채택)** — 다중 source 의 existing·additional URL 모두 보존 + 검증 분기 라우팅 가능, (b) 우선 source 단일 채택 — 정보 손실, (c) feature_selection UI 에서 두 source 카드 별도 노출 — UX 부담                                                                                                                                        |
| **D24** *(turn-11 신설, [x] 확정, v0.10.27 적용)* | 통합 노드 내부 page meta 단계 부분 실패 정책                            | **(a) 부분 실패 허용 + LLM 입력에 빈 메타 carry-through** — `_feature_mapping_runner.run_source_mapping` 의 단계 1 (`_build_candidates_with_meta`) 가 개별 URL HTTP fail 시 `page_title=""` 로 carry, LLM 이 not_found 판정. v0.10.24 의 `body_excerpt` 도 동일 정책 — 빈 값 carry-through                                                                      | (a) **부분 허용 + carry-through (채택)**, (b) 임계 기반 노드 fail (예: page_meta fail 비율 ≥ 50% 시 노드 전체 fail), (c) 모든 URL 성공 시에만 LLM 진입 — 너무 보수적                                                                                                                                                           |
| **D25** *(turn-11 신설, [x] 확정, v0.10.27 적용)* | 5개 통합 노드의 헬퍼 모듈 위치                                        | **(a) `feature_url_mapper_node.py` 유지 + 통합 노드 5종 공유 import** — `_feature_mapping_runner.py` 가 5 thin wrapper 노드 의 공통 동작 흡수. v1.0 시점에 명시적 helper 파일 분리 재검토                                                               | (a) **`feature_url_mapper_node.py` 유지 (채택)**, (b) `_feature_url_mapper_helpers.py` 명시적 helper 파일로 분리, (c) 5개 통합 노드 각각에 동일 헬퍼 복제 — 코드 중복 위험                                                                                                                                        |
| **D26** *(turn-11 신설, [x] 확정, v0.10.27 적용)* | 통합 노드 명명                                                  | **(a) `feature_mapping_<source>_node` 유지** — v0.10.27 의 5 통합 노드 모두 본 패턴 채택. `_feature_mapping_runner.run_source_mapping(source, ...)` thin wrapper 로 통일                                                                                                                         | (a) **`feature_mapping_<source>_node` 유지 (채택)** — 변경 표면적 작음, (b) `source_pipeline_<source>_node` — 책임 정확하나 mapping 표현 손실, (c) `meta_and_mapping_<source>_node` — 길어짐                                                                        |
...
| **D37** *(turn-54 신설, [x] 확정, turn-66 적용)* | 발행일 ≤ 36개월 검증 방식 | **(c→b 적용) v0.10.24 에서 `_fetch_meta` 의 `<meta article:published_time>` + `<time datetime>` 추출 + `_is_recent_enough` 헬퍼 + `_feature_mapping_runner` 의 blog_community 분기 적용** | (a) Brave 결과 `age` 필드 — 필드 없음, (b) **v0.10.24 도입 — `_MetaExtractor` 보강 + `_is_recent_enough(max_months=36)` 적용 (채택)** — published_at 부재·파싱 실패 시 안전 통과, (c) ~~v0.10.22b 생략~~ — v0.10.24 에서 도입 완료 |
...
| **D39** *(turn-56 신설, [x] 확정)* | 단일 prompt → 5 source-type prompt 분배 정책 | **(b) 공통 base 정적 복사 + source-type 차별화** | (a) 전면 복사 + 분기 추가 — 5 파일 동기화 부담 ↑, (b) **공통 정책 정적 복사 + source-type 차별 정책 명확 분리 (채택)** — 약 5×100~150줄, 일관성 보장, (c) 공통 base + extension prompt 분리 — LLM 호출 시 2 파일 로드 복잡도 ↑ |
| **D40** *(turn-56 신설, [x] 확정)* | output.schema 분리 정책 | **(a) 5 디렉토리 각자 schema 신설** | (a) **각 `agents/feature_mapping_<source>/output.schema.json` 신설 + source 특수 메타 (subpage_category·domain_class·view_count·platform·source_tier) 정의 — 기존 agents/ 패턴 정합 (채택)**, (b) 공통 base schema 1개 + 5 prompt referencing — schema 로드 경로 차이 발생 |
| **D41** *(turn-56 신설, [x] 확정)* | 기존 `agents/feature_url_mapper/output.schema.json` 의 origin enum 갱신 시점 | **(b) 5 새 schema 와 동시 일괄 갱신** | (a) 5 새 schema 만 6종 enum + 기존 단일 schema 변경 안 함 — v0.10.27 도입 전까지 단일 prompt 후방 호환 어려움, (b) **기존 + 5 새 schema 모두 6종 enum 일괄 (채택)** — v0.10.27 도입 전까지 단일 prompt 호출 시 새 origin 값 통과 |
| **D42** *(turn-60 신설, [x] 확정, turn-62 hotfix)* | v0.10.27 와 v0.10.25 의 진행 순서 | **(a) v0.10.27 → v0.10.25 + 임시 `_union_raw_features` 헬퍼.** v0.10.27.1 hotfix 에서 priority dedup → **candidate_coverage union 으로 즉시 개선** (Issue 3 해소). v0.10.25 정식화 시 본 임시 헬퍼 폐기 후 `additional_urls_validation_node` 의 정식 함수로 이관. | (a) **v0.10.27 PR 내 임시 헬퍼 + v0.10.27.1 hotfix (채택)** — youtube_reactions feature 가 blog_community 와 동일 (rt, fid) 일 때 priority dedup 으로 폐기되어 UI 미렌더링 회귀 (Issue 3). hotfix 후 두 source 의 URL 이 동일 feature 카드 내에 통합 렌더링. (b) 단일 PR 통합 — 약 +600줄 review 부담, (c) v0.10.25 선행 + 5종 호환 어댑터 → v0.10.27 — 시리즈 의존 순서 역전 |
| **D43** *(turn-60 신설, [x] 확정)* | 단일 `agents/feature_url_mapper/` 폐기 시점 | **(a) v0.10.27 PR 내 폐기** | (a) **v0.10.23 cleanup 패턴 정합 + turn-56 사용자 권장 (채택)**, (b) v0.10.27 도입 후 옛 호환 유지 — dead code, (c) v0.10.27.1 별도 cleanup PR — PR 수 증가 |
| **D44** *(turn-60 신설, [x] 확정)* | 5 통합 노드 내부 2단계 캐시 정책 | **(a) 설계 §5-6a 그대로 채택** | (a) **단계 1 page meta: `_fetch_meta` 의 agent_id="page_meta_collect" 24h TTL 캐시 공유 / 단계 2 LLM 매핑: agent_id=`feature_mapping_<source>` + prompt_version=`feature_mapping_<source>:v0.10.27` 분리 (채택)**, (b) 옛 단일 캐시 키 공유 — source 별 invalidate 어려움, (c) hybrid — 일관성 ↓ |
| **D46** *(turn-63 신설, [x] 확정)* | feature_selection_node 의 origin 라벨 + source 특수 메타 표시 정책 | **(a) origin 6종 + macro 의 source_tier 추가 분기 (총 7종 UI 라벨) + source 특수 메타 9건 carry-through** | (a) **6종 origin + source_tier 분기 (공식·공식 sub-page·블로그·커뮤니티·YouTube·운영 채널·공식 통계·뉴스 보강) — UI chip 색상 차별화 + source 특수 메타 (subpage_category·domain_class·view_count·platform·source_tier·tier_group) 모두 carry (채택)**, (b) origin 2종 (공식 / Brave) 유지 — 의미 손실 ↑, (c) origin 변경 + 메타 손실 — 회귀 위험 |
| **D47** *(turn-63 신설, [x] 확정)* | macro candidate 라벨 변환 위치 | **(c) hybrid — server payload 에 candidate_label 부착 + client fallback** | (a) server `feature_selection_node` 만 변환 — label 정책 중앙화하나 client 라벨 매핑 추가 어려움, (b) client 만 분기 — label 정책 산재, (c) **server 가 `candidate_label` 부착 (macro → "산업·시장 데이터") + client 가 own_*/comp_* 라벨 매핑 추가 가능 (채택)** — 변경 최소 + 미래 확장 용이 |
| **D48** *(turn-65 신설, [x] 확정)* | reaction_insight 의 youtube_reactions URL 다수 시 truncate 정책 | **(a) 대표 1건 (view_count 최대) + `youtube_truncated_count` 메타** | (a) **view_count 내림차순 1건 채택 + 나머지 N-1 건 메타로 표기 + UI "+ N건 추가 영상" pill (채택)** — 사용자 의도 정합 + 가독성 ↑, (b) 대표 3건 + N건 — 카드 폭주 회피 미흡, (c) 펼치기 토글 — 복잡도 ↑ |
| **D49** *(turn-65 신설, [x] 확정)* | truncate 적용 범위 | **(a) `youtube_reactions` origin 만** | (a) **사용자 의도 (영상 갯수 多) 한정 (채택)** — 다른 source URL 은 정보량 적어 truncate 불필요, (b) 모든 origin 일관 적용 — 정보 손실 |
| **D50** *(turn-65 신설, [x] 확정)* | truncate 처리 위치 | **(a) server `feature_selection_node` 정렬 + truncate** | (a) **server 가 view_count 정렬 + 1건 채택 + youtube_truncated_count 부착 (채택)** — 정책 중앙화 + payload 크기 ↓, (b) client 처리 — 정책 산재 + 모든 URL 전송 부담, (c) hybrid — 일관성 ↓ |
| **D45** *(turn-64 신설, [x] 확정)* | `feature_mapping_owned_channels_node` 의 LLM 호출 정책 | **(a) LLM 호출 생략 + URL carry-through + marketing_social UI 재정의** | (a) **owned_channels 통합 노드의 LLM 호출 생략. `owned_channel_urls_by_candidate` 를 `_carry_owned_channels` 헬퍼로 candidate × platform placeholder 로 변환하여 `owned_channel_raw_features` 에 carry. feature_selection_node 가 `owned_channels_card` payload (candidate × platform 6종 매트릭스) 산출. client `OwnedChannelCard` 컴포넌트가 marketing_social 카드 의 안내 박스 아래에 별도 렌더링. feature 카드 자체는 B-only 형식 (URL coverage 영역 숨김 + 체크박스 비활성). 안내 문구 "공식 채널 URL 을 식별한 뒤" 로 단계 분리 명시 (채택)** — 사용자 의도(turn-49 owned_channels 책임 범위) 정합 + LLM 토큰 절감, (b) LLM 호출 유지 + 출력 schema 변경 (candidate × platform 그룹화) — schema 5종 일관성 깨짐, (c) LLM 호출 유지 + UI 분기만 — feature × candidate × URL 매핑이 의미 없는 LLM 호출 발생 |
...
| 버전 | 일자 | 변경 내용 | 비고 |
|:-:|---|---|---|
| 2.1 | 2026-06-04 | **본 시리즈 종료 + 문서 정리 (turn-68)** — (1) §10 결정 항목 표 정리: 본 시리즈 코드 차원 적용 완료된 9개 항목 (D14·D15·D16·D17·D19·D20·D22·D24·D25·D26) 에 `[x] 확정` + 적용 PR 표기 추가. 실제 코드 적용 완료 상태였으나 §10 표기만 누락된 문서 부채 해소. (2) **본 시리즈 (v0.10.18 ~ v0.10.28c + v0.10.24 + v0.10.25) 코드 완료 선언** — `feature_url_mapper_redesign.md` 의 모든 핵심 정밀화 PR 적용 + 5중 fan-out 1차/2차 토폴로지 + cross_reference + 5 통합 노드 + 정식 `_union_raw_features` + 5 source 검증 분기 + UI 재정의 모두 정식 적용. (3) **후속 작업 분리 명시** — (a) v0.10.18.1 (taxonomy 캐시 schema-aware 보강 — D28 부채) + (b) v0.11 (playwright SPA fallback — D19 적용 PR) 두 PR 은 **feature_extraction 및 분석 탭 4종 리포트 작성 후 진행** (사용자 결정 turn-68). (4) **§10 결정 대기 잔존 0건** — D18·D21·D23·D27~D50 + D14~D17·D19·D20·D22·D24~D26 모두 [x] 확정. D27 (taxonomy 캐시 schema-aware) 와 D28 (동일) 만 v0.10.18.1 PR 의 실 적용 대기. (5) **`pipeline_topology_redesign.md` §6-5 정리** — feature_url_mapper 노드 설계 변경 사항을 간략 요약 + 상세는 본 문서 참조 안내 (별도 commit task). (6) 영향 파일: `docs/design/feature_url_mapper_redesign.md` (§10 9 항목 [x] 표기 + §12 v2.1 entry, +30) · `docs/design/pipeline_topology_redesign.md` (§6-5 간략화 + 본 문서 참조, +20/-N). | **본 시리즈 종료 (DRAFT → 확정 표기)** — §10 결정 대기 0건. 후속 v0.10.18.1·v0.11 별도 시리즈 |
| 2.0 | 2026-06-04 | **v0.10.25 — `additional_urls_validation_node` 정식화 + D23 정식 union + 5 source 검증 분기 (turn-67)** — (1) **본 시리즈 임시 → 정식 전환 마무리**: v0.10.27.1 hotfix 의 `_union_raw_features` (feature_url_mapper_node.py 의 임시 헬퍼) 폐기 + `additional_urls_validation_node.py` 내부에 정식 헬퍼 신설. (2) **D23 [x] 정식 적용** — candidate_coverage union (URL dedup + coverage 갱신) + **`source_origin` 메타 부착** (`additional_urls` 의 검증 분기 키). (3) **5 source 별 검증 분기 신설** — `_validate_youtube_video` (videos.list API · 1 unit/URL · view_count·like·comment 메타 보강 · 삭제·private 영상 validated=False) · `_validate_owned_channel` (HEAD/GET + is_brand_match fuzzy 검증 · threshold 0.7) · `_validate_official_subpage` (HEAD/GET) · `_validate_blog_community` (HEAD/GET + 발행일 36개월 재검증 `_is_recent_enough`) · `_validate_macro_subpage` (HEAD/GET + 화이트리스트 매칭 · Tier 1·2·뉴스 17 도메인). (4) **youtube_client.py 의 `youtube_videos_list(video_id)` public 함수 신설** — 단일 video_id 의 statistics 메타 조회 (1 unit, cache 미적용 — view_count 변동 빠름). (5) **임시 fallback 폐기** — `state["raw_features"]` 옛 키 사용 제거. v0.10.27 정식화 완료로 옛 흐름 무관. (6) **옛 `_validate_additional_urls` 함수 폐기** (feature_url_mapper_node.py) — `_validate_additional_urls_v2` (additional_urls_validation_node.py) 가 source_origin 메타 기반으로 정식 분기. (7) **5종 system_prompt 갱신** — additional_urls 출력의 `source_origin` 필드 명시 (각 source 별 고정값: `"official_subpage"` · `"blog_community"` · `"youtube_reactions"` · `"owned_channel_search"` · `"macro_search"`). (8) **§10 D23 [x] 적용** (turn-11 신설 → turn-67 정식 적용). (9) **v0.10.27.1 임시 헬퍼 회귀 검증** — 6 케이스 (단일 source · ★ blog+youtube URL union + coverage 갱신 · 다른 candidate_id append · URL dedup · 빈 state · 5 source 결합) 정식 헬퍼에서도 동일 동작 보장. (10) **D38 (본문 길이 ≥ 200자) 위임** — 본 PR 에서는 미적용 (v0.10.27 통합 노드의 page_meta 단계 책임 + body_excerpt 가 v0.10.24 의 신규 메타이므로 v0.10.25 의 향후 점진 검토). (11) 영향 파일: `server/graph/nodes/additional_urls_validation_node.py` (전면 재작성, 500줄) · `server/graph/nodes/feature_url_mapper_node.py` (-130, 임시 `_union_raw_features` 폐기 + 옛 `_validate_additional_urls` 폐기) · `server/llm/youtube_client.py` (+90, `youtube_videos_list` 신설) · `agents/feature_mapping_{official,blog_community,youtube_reactions,owned_channels,macro}/system_prompt_kr.md` (+5 × 5 = +25, `source_origin` 필드 명시) · 본 문서 (+30). 합계 약 +500/-130 (순증가 +370). | DRAFT, §10 D14~D17·D19·D20·D22·D24~D26 결정 대기 (D18·D21·D23·D27~D50 [x] 확정 + D23 정식 적용) |
| 1.9 | 2026-06-04 | **v0.10.24 — `_fetch_meta` body 보강 + D37 발행일 검증 적용 (turn-66)** — (1) **`_MetaExtractor` 보강** — `<h1>`/`<h2>` 헤더 list 추출 (페이지 목차) + `<body>` 텍스트 노드 첫 800자 (script/style/nav/footer/header/aside 필터) + 발행일 추출 (`<meta article:published_time>` · `<meta og:published_time>` · `<meta name="pubdate">` · `<time datetime>`). (2) **`_fetch_meta` cache schema bump** — `cache_context["v"]: 1→2` 변경으로 기존 캐시 자동 무효화. 출력 dict 에 `h1_h2`·`body_excerpt`·`published_at` 3 키 추가. (3) **`_build_candidates_with_meta` 신규 메타 carry** — official_source 와 brave_search URL 모두에 신규 메타 키 전파 (Brave URL 은 5 통합 노드의 page meta 단계에서 `_fetch_meta` 호출 시 채워짐). (4) **`_is_recent_enough(published_at, max_months=36)` 헬퍼 신설** — ISO 8601 파싱 (Z 접미사 처리) + UTC 타임존 통일 + 30일=1개월 근사. published_at 부재·파싱 실패 시 안전 통과. (5) **`_feature_mapping_runner` blog_community 분기 발행일 검증** — `candidates_with_meta` 산출 직후 validated_urls 의 36개월 초과 항목 자동 제외 + 제외 건수 log. (6) **3종 system_prompt 갱신** (official·blog_community·macro) — 입력 구조 절에 `h1_h2`·`body_excerpt` (·blog_community 한정 published_at) 정의 추가 + 판정 기준 절에 `h1_h2` 헤더 매칭 + `body_excerpt` 본문 수치/의견 확인 정책 명시. youtube_reactions·owned_channels 는 `_fetch_meta` 미사용 (각 YouTube API / URL carry-through) — 미적용. (7) **§10 D37 [x] 적용 표기** (turn-54 위임 → turn-66 적용). (8) **D37 부채 해소** — v0.10.22b 의 5가지 정밀화 책임 중 발행일 검증이 본 PR 에서 정식 도입됨. (9) **§9 v0.10.24 entry 갱신** (신설 → 실 구현, +40줄 → +250줄). (10) 영향 파일: `server/graph/nodes/feature_url_mapper_node.py` (+125, `_MetaExtractor` 보강 + `_fetch_meta` bump + `_is_recent_enough` 신설 + `_build_candidates_with_meta` carry + `from datetime import timedelta` 추가) · `server/graph/nodes/_feature_mapping_runner.py` (+20, `_is_recent_enough` import + blog_community 분기 발행일 검증) · `agents/feature_mapping_official/system_prompt_kr.md` (+10) · `agents/feature_mapping_blog_community/system_prompt_kr.md` (+15, body 보강 + published_at 명시) · `agents/feature_mapping_macro/system_prompt_kr.md` (+10) · 본 문서 (+30). 합계 약 +210. | DRAFT, §10 D14~D17·D19·D20·D22~D26 결정 대기 (D18·D21·D27~D50 [x] 확정 + D37 적용) |
| 1.8 | 2026-06-04 | **v0.10.28c — youtube_reactions URL 대표 1건 + N건 추가 영상 표기 (turn-65)** — (1) **Issue 3 UX 보강** (turn-prev): reaction_insight 카드의 youtube_reactions URL 다수 (5+) 시 카드 폭주 → 가독성 저하. (2) **D48·D49·D50 결정 일괄 확정** — D48 (a) view_count 정렬 + 대표 1건 + youtube_truncated_count 메타 · D49 (a) youtube_reactions origin 만 truncate · D50 (a) server 처리 + 정책 중앙화. (3) **server 변경 — `_truncate_youtube_reactions` 헬퍼 신설**: existing_urls 중 origin="youtube_reactions" URL 만 분리 → view_count 내림차순 정렬 → 대표 1건 채택 (`youtube_truncated_count = N-1` 메타 부착) → 비-youtube URL 그대로 유지. youtube ≤ 1건이면 truncate 미적용 (변동 없음). `_build_feature_items_from_analysis` 의 existing_urls 변환 직후 본 헬퍼 호출. (4) **client 변경 — `FeatureSelectionPage.jsx`**: origin chip 다음 위치에 `u.youtube_truncated_count > 0` 분기로 "+ N건 추가 영상" 빨강 pill 표시 (border + bg-red-50 + text-red-700). (5) **§10 D48·D49·D50 신설 [x] 확정**. (6) 영향 파일: `server/graph/nodes/feature_selection_node.py` (+30, `_truncate_youtube_reactions` 헬퍼 + `_build_feature_items_from_analysis` 호출 한 줄) · `client/src/components/FeatureSelectionPage.jsx` (+8, pill 분기) · 본 문서 (+25). 합계 약 +63. (7) **검증** (격리, 6 케이스 모두 PASS): youtube 0건 · 1건 (truncate 미적용) · ★ 3건 → 대표 v2 + count 2 · 5건 view_count 정렬 · None view_count 처리 · 다른 origin 무영향 (D49). (8) **UI 변화 (운영 검증 예상)**: 영상 5+ 건 케이스에서 1 URL + "+ 4건 추가 영상" 빨강 pill 표시. view_count 최대 영상이 대표로 채택 → 시청자 반응 풍부한 영상 우선 노출. | DRAFT, §10 D14~D17·D19·D20·D22~D26 결정 대기 (D18·D21·D27~D50 [x] 확정) |
| 1.7 | 2026-06-04 | **v0.10.28b — owned_channels LLM 호출 생략 + marketing_social UI 재정의 (turn-64)** — (1) **Issue 2 해소** (turn-prev): owned_channels 통합 노드의 LLM 호출이 의미 없음 (URL 1개로 SNS 게시물 빈도 판정 불가). 사용자 의도 (turn-49) 는 candidate 별 공식 채널 URL 식별까지가 책임, feature 값 산출은 v1.0 §6-6a 책임. (2) **D45 (a) 채택** — owned_channels LLM 호출 생략 + URL carry-through + marketing_social UI 재정의. (3) **server 변경 — `_feature_mapping_runner._carry_owned_channels`**: source='owned_channels' 분기로 LLM 호출 우회. `owned_channel_urls_by_candidate` 를 candidate 별 platform 수 기반 coverage 산정 (≥3 sufficient, ≥1 partial, 0 not_found) + 단일 placeholder feature (`feat_owned_channels_summary`) 로 `owned_channel_raw_features` 산출. (4) **server 변경 — `feature_selection_node`**: `_REPORT_INTRO_TEXTS["marketing_social"]` "공식 채널 URL 을 식별한 뒤" 로 단계 분리 명시. `_build_reports_payload` 의 marketing_social 분기 — features 를 B-only 형식 (taxonomy 기반·URL 영역 숨김) + `owned_channels_card` payload 부착. `_build_owned_channels_card(state)` 헬퍼 — `own_product` + `competitor_candidates` 의 product_name 매핑 + 6 platform 순서 (`_PLATFORM_ORDER`: instagram·x·youtube_official·blog_naver·blog_tistory·press_release) + platform 별 found/handle/account_scope/channel_id/subscriber_count carry. (5) **client 변경 — `OwnedChannelCard.jsx` 신설** (120줄): candidate × platform 그리드 컴포넌트. own=인디고 / competitor=앰버 chip + ✅/❌ found 표시 + account_scope 한국어 라벨 (모회사·서브 브랜드·본 상품·지역 한정) + 구독자 수 formatting (만/천). (6) **client 변경 — `FeatureSelectionPage.jsx`**: `OwnedChannelCard` import + ReportSection 의 `isMarketingSocial` 분기. checkboxDisabled 가 `isBOnly || isMarketingSocial` 로 통합 — marketing_social feature 카드의 체크박스 비활성 + 자동 포함 표시. 안내 박스 아래에 `<OwnedChannelCard card={ownedChannelsCard} />` 렌더링. (7) **§10 D45 [x] 확정**. (8) `Future_Improvements.md` §3 (v0.10.28b 사양 보관) → 구현 완료 표기. (9) 영향 파일: `server/graph/nodes/_feature_mapping_runner.py` (+125, owned_channels 분기 + `_carry_owned_channels`) · `server/graph/nodes/feature_selection_node.py` (+130, `_REPORT_INTRO_TEXTS` + marketing_social 분기 + `_build_owned_channels_card` + `_PLATFORM_ORDER`·`_PLATFORM_LABELS`) · `client/src/components/OwnedChannelCard.jsx` (신설, 120줄) · `client/src/components/FeatureSelectionPage.jsx` (+25/-7, OwnedChannelCard import + ReportSection 분기) · 본 문서 (+30) · `docs/design/Future_Improvements.md` (§3 표기 갱신). 합계 약 +430/-7. | DRAFT, §10 D14~D17·D19·D20·D22~D26 결정 대기 (D18·D21·D27~D47 [x] 확정 + D45 확정) |
| 1.6 | 2026-06-04 | **v0.10.28a — macro candidate 라벨 + source 메타 carry + origin chip 6종 분기 (turn-63)** — (1) **Issue 1 해소** (turn-prev): macro candidate_id="macro" 가 UI 에 그대로 노출되어 사용자 친화성 ↓ + origin chip 이 brave_search/그 외 2종만 분기되어 source_tier 손실. (2) **D46·D47 결정 일괄 확정** — D46 (a) origin 6종 + macro source_tier 분기 = 7종 UI 라벨 + source 특수 메타 9건 carry · D47 (c) hybrid — server payload 에 candidate_label 부착 + client fallback. (3) **server 변경** — `feature_selection_node._build_feature_items_from_analysis`: existing_urls 에 `source_tier`·`tier_group`·`subpage_category`·`domain_class`·`view_count`·`platform` 메타 carry + coverage_details 에 `candidate_label` 부착. `_candidate_label("macro") → "산업·시장 데이터"` 변환. `_CANDIDATE_LABEL_OVERRIDES` 정적 dict (확장 용이). (4) **client 변경** — `FeatureSelectionPage.jsx:CoverageDetails`: `originChip(u)` 함수 신설 (origin × source_tier 별 7종 분기 + 색상 차별화: 공식=회색 · 공식 sub-page=회색 · 블로그·커뮤니티=teal · YouTube=빨강 · 운영 채널=핑크 · 공식 통계=보라 · 뉴스 보강=주황 · Brave=파랑). candidate 헤더에 `cov.candidate_label || cov.candidate_id` fallback + macro 시각 구분 (font-sans + 보라). 옛 brave_search/그 외 2종 삼항 분기 폐기. (5) **§10 D46·D47 신설 [x] 확정**. (6) 영향 파일: `server/graph/nodes/feature_selection_node.py` (+30, `_CANDIDATE_LABEL_OVERRIDES` + `_candidate_label` + existing_urls 메타 carry + coverage_details candidate_label) · `client/src/components/FeatureSelectionPage.jsx` (+40/-4, originChip 함수 + candidate 헤더 분기 + chip 라벨 변경) · 본 문서 (+25). 합계 약 +95/-4. (7) **검증** (격리, 모두 PASS): `_candidate_label` 4 케이스 (macro 변환 + own_*/comp_* fallback) · `_build_feature_items_from_analysis` 메타 9 키 carry + macro/comp candidate_label 정확 + source_tier 보존 · JSX 정규식 17 체크 (originChip 7 분기 + 라벨 6종 + candidate 헤더 + 옛 분기 폐기). (8) **UI 변화 (운영 검증 예상)**: market_context_swot 카드의 macro candidate 가 "산업·시장 데이터" 라벨 + 보라색 시각 구분 + URL chip 이 `[공식 통계]` / `[뉴스 보강]` 분기. blog_community 의 `domain_class` · YouTube `view_count` 등 메타가 후속 PR (v0.10.28c) 에서 UI 활용 가능하도록 사전 carry. | DRAFT, §10 D14~D17·D19·D20·D22~D26 결정 대기 (D18·D21·D27~D47 [x] 확정) |
| 1.5 | 2026-06-04 | **v0.10.27.1 hotfix — `_union_raw_features` candidate_coverage union 으로 개선 (turn-62)** — (1) **회귀 진단**: v0.10.27 임시 헬퍼의 priority dedup 정책이 동일 (report_type, feature_id) 산출 시 우선 source 1건만 채택 + 후속 source 결과 폐기. blog_community 가 youtube_reactions 보다 우선이므로 `reaction_insight` 의 동일 feature 가 양쪽에서 산출되면 youtube_reactions 폐기 → UI 에 YouTube 영상 URL 0건 (Issue 3 미렌더링). (2) **hotfix**: priority dedup 폐기 + candidate_coverage union 도입. 동일 (rt, fid) 가 여러 source 산출 시 첫 등장 source 의 feature 메타 유지 + `existing_urls`·`additional_urls` URL union (dedup) + `coverage` 갱신 (sufficient > partial > not_found). (3) **영향 파일**: `server/graph/nodes/feature_url_mapper_node.py` (`_union_raw_features` 함수 재작성, 60줄). (4) **검증** (격리, 6 케이스 모두 PASS): 단일 source · ★ blog+youtube URL union + coverage 갱신 (Issue 3 핵심) · 다른 candidate_id append · URL dedup · 빈 state · 5 source 결합. (5) **§10 D42 갱신** — (a) 채택 옵션에 v0.10.27.1 hotfix 명시. v0.10.25 정식화 시 본 임시 헬퍼 폐기. (6) **UI 변화 (운영 검증 예상)**: reaction_insight 카드의 동일 feature 내에 `[blog]` + `[YouTube]` URL 모두 표시. coverage 가 partial → sufficient 갱신되어 "충분" 비율 ↑. | DRAFT, §10 D14~D17·D19·D20·D22~D26 결정 대기 (D18·D21·D27~D44 [x] 확정, D42 hotfix 적용) |
| 1.4 | 2026-06-04 | **v0.10.27 — 5중 fan-out (2차) 통합 노드 5종 신설 실 구현 (turn-60)** — (1) **5 통합 노드 신설**: `feature_mapping_official_node` · `_blog_community_node` · `_youtube_reactions_node` · `_owned_channels_node` · `_macro_node`. 각 노드는 `_feature_mapping_runner.run_source_mapping(source, ...)` 만 호출하는 thin wrapper. (2) **D42·D43·D44 결정 일괄 확정** — D42 (a) v0.10.27 → v0.10.25 + 임시 `_union_raw_features` 헬퍼 · D43 (a) 단일 `agents/feature_url_mapper/` 폐기 · D44 (a) 설계 §5-6a 2단계 캐시 정책. (3) **3 노드 파일 삭제** — `urls_merge_node.py`·`page_meta_collect_node.py`·`feature_mapping_llm_node.py` (헬퍼는 `feature_url_mapper_node.py` 모듈에 잔존). (4) **`agents/feature_url_mapper/` 디렉토리 폐기** — system_prompt + output.schema 전체 삭제. v0.10.23 의 5 prompt + schema 만 활용. (5) **state.py 5종 `*_raw_features` 키 신설** — `official_raw_features`·`blog_community_raw_features`·`youtube_reactions_raw_features`·`owned_channel_raw_features`·`macro_raw_features`. 옛 `candidates_with_meta`·`raw_features` 키는 v0.10.27 임시 호환용으로 docstring 만 갱신 (TypedDict 정의는 잔존). (6) **임시 `_union_raw_features` 헬퍼 (D42 a)** — `feature_url_mapper_node.py` 에 신설. priority 기반 (official > blog_community > youtube_reactions > owned_channels > macro) dedup union. v0.10.25 정식 헬퍼 (candidate_coverage union) 도입 시 폐기. (7) **`additional_urls_validation_node` 임시 호환** — 5종 `*_raw_features` 를 `_union_raw_features` 로 단일 raw_features 변환 후 기존 `_validate_additional_urls` 호출. `state["raw_features"]` fallback (옛 그래프 호환). (8) **graph.py 토폴로지 변경** — cross_reference → 5중 fan-out (2차) → list-fan-in barrier (2차) → additional_urls_validation. 검증 변수 `_fanout_5_1`·`_fanin_5_1`·`_fanout_5_2`·`_fanin_5_2` 4종. 옛 `_e_um`·`_e3` 등 폐기. (9) **5 통합 노드 공통 동작 (`run_source_mapping`)** — 단계 1: `_build_candidates_with_meta` (official source 만 official_sources carry) → 단계 2: report_type 별 병렬 LLM 호출 (`_filter_candidates_for_report` + `ClaudeCodeCliAnalyzer` + 자기 source schema). 캐시 정책 (D44 a): `agent_id=f"feature_mapping_{source}"`, `prompt_version=f"feature_mapping_{source}:v0.10.27"`. (10) **§9 v0.10.27 entry 갱신** (신설 → 실 구현, +400줄 → +850/-680). (11) **§10 D42·D43·D44 [x] 확정**. (12) 영향 파일: `server/graph/nodes/_feature_mapping_runner.py` (신설, 280줄) · `server/graph/nodes/feature_mapping_{official,blog_community,youtube_reactions,owned_channels,macro}_node.py` (신설 × 5, 각 30줄) · `server/graph/nodes/feature_url_mapper_node.py` (+50, `_REPORT_TYPES_BY_SOURCE` + `_union_raw_features`) · `server/graph/nodes/additional_urls_validation_node.py` (+20, 임시 union 어댑터) · `server/graph/graph.py` (+30/-20, 토폴로지 변경 + 검증 변수) · `server/graph/state.py` (+50/-10, 5종 키 + 폐기 표시) · `server/graph/nodes/urls_merge_node.py` 삭제 · `_page_meta_collect_node.py` 삭제 · `_feature_mapping_llm_node.py` 삭제 · `agents/feature_url_mapper/` 디렉토리 전체 삭제 · 본 문서 (+50). 합계 약 +850/-680, 순증가 +170. | DRAFT, §10 D14~D17·D19·D20·D22~D26 결정 대기 (D18·D21·D27~D44 [x] 확정) |
| 1.3 | 2026-06-04 | **v0.10.26 — `cross_reference_node` 신설 실 구현 (turn-58)** — (1) §5-6 의 결정론적 후처리 필터링 노드 신설. youtube_reactions × owned_channels(youtube_official) cross-reference. (2) **위치**: 5중 fan-out 1차 list-fan-in barrier 직후 + urls_merge 직전. (3) **동작**: owned_channels 의 `platform="youtube_official"` 항목의 `channel_id` 집합 추출 → reactions 영상 중 일치 항목 제외. owned_channels 결과는 변경 없이 통과. (4) **graph.py 토폴로지 변경**: 5중 list-fan-in target 을 `urls_merge` → `cross_reference` 로 변경 + `cross_reference → urls_merge` edge 신설. graph.py docstring 다이어그램 + 검증 출력 (`_fanin_5`·`_e_cr` 변수) 동시 갱신. (5) **page_meta_collect 폐기 이연** — 설계 문서 §9 의 옛 v0.10.26 entry 의 "page_meta_collect 폐기 (헬퍼만 잔존)" 부분은 v0.10.27 통합 노드 도입 시점으로 이연. 본 PR 에서는 `cross_reference → urls_merge → page_meta_collect` 흐름 유지. (6) **결정론적 + LLM 미사용 + 캐시 미사용** — wall-clock < 50ms. set 추출 + list comprehension 만. (7) graceful 종료 — youtube_reactions 빈 dict / owned_channels 빈 dict / youtube_official channel_id 0건 모두 정상 통과. 실패 가능 분기 없음. (8) **§9 v0.10.26 entry 갱신** (신설 → 실 구현, +80줄 → +130줄). (9) **영향 파일**: `server/graph/nodes/cross_reference_node.py` (신설, 130줄) · `server/graph/graph.py` (+15, import + add_node + edge + 검증 변수) · 본 문서 (+30). 합계 약 +175. | DRAFT, §10 D14~D17·D19·D20·D22~D26 결정 대기 (D18·D21·D27~D41 [x] 확정) |
| 1.2 | 2026-06-04 | **v0.10.23 — 5종 system_prompt + 5종 output.schema 분배 실 구현 (turn-56)** — (1) 단일 `agents/feature_url_mapper/system_prompt_kr.md` (117줄·7종 report_type 모두 처리) 를 5 source-type 별로 분배. (2) **D39·D40·D41 결정 일괄 확정** — D39 (b) 공통 base 정적 복사 + source-type 차별화 · D40 (a) 5 디렉토리 각자 schema 신설 · D41 (b) 기존 + 5 새 schema 모두 origin enum 6종 일괄 갱신. (3) **5 prompt 신설** (각 100~150줄): `feature_mapping_official` (comparison_matrix · battlecard A Fact · market_context_swot 규제 + subpage_category 활용) · `feature_mapping_blog_community` (reaction_insight + domain_class 가중치) · `feature_mapping_youtube_reactions` (reaction_insight + view/like/comment_count 임계 가중치) · `feature_mapping_owned_channels` (marketing_social · battlecard 광고 카피 + platform·account_scope 매핑) · `feature_mapping_macro` (market_context_swot 매크로 + source_tier·tier_group 가중치 + candidate_id="macro" 단일 키). (4) **5 schema 신설** — 각자 담당 report_type enum 1~3종 + source 특수 메타 필드 정의. (5) 기존 `agents/feature_url_mapper/output.schema.json` 의 origin enum 2종 → 6종 갱신 (D41 b — v0.10.27 도입 전까지 단일 prompt 호출 후방 호환). (6) **§5-6a 통합 노드 책임 명확화** — 본 5 prompt 가 v0.10.27 통합 노드 (`feature_mapping_<source>_node`) 의 LLM 호출 시점에 시스템 메시지로 주입되어 LLM 출력의 동작 규칙·출력 형식·정책을 결정. (7) **§10 D39~D41 신설 [x] 확정**. (8) 영향 파일: `agents/feature_mapping_official/{system_prompt_kr.md,output.schema.json}` (신설 130+50줄) · `agents/feature_mapping_blog_community/{...}` (115+50) · `agents/feature_mapping_youtube_reactions/{...}` (120+50) · `agents/feature_mapping_owned_channels/{...}` (140+50) · `agents/feature_mapping_macro/{...}` (135+50) · `agents/feature_url_mapper/output.schema.json` (+1, origin enum 6종) · 본 문서 (+40). 합계 약 +1,180/-5. | DRAFT, §10 D14~D17·D19·D20·D22~D26 결정 대기 (D18·D21·D27~D41 [x] 확정) |
