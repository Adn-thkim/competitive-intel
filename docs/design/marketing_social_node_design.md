...
| MS-D14 | YouTube **핸들 프로브 폴백** — Brave 가 채널을 노출하지 않는 케이스(실측: @travelwallet, 두 쿼리 40건 전부 무관)는 발견된 타 platform 핸들(.official 제거형)·도메인 SLD 에서 영문 핸들 후보를 유도해 `channels.list?forHandle=` 로 실존+신원(채널명 브랜드 토큰) 검증 후 채택 (conf 0.75, origin=youtube_handle_probe, 후보당 1 unit·상한 4). 핸들은 전역 고유라 오탐은 채널명 검증으로 차단 | **확정** (2026-06-07, 사용자) |
| MS-D15 | RSS·sitemap 피드 경로는 robots.txt 체크 면제 — 구독·배포 목적 공개 자원. 트래블월렛 네이버 블로그가 robots 차단으로 미수집되던 사례 해소 (2026-06-07 실측) | **확정** (2026-06-07, 실측) |
| MS-D16 | 피드 200 도달했으나 게시물 0건 = `measured_empty` (측정·0건) — 인스타·X(수집 시도 자체 안 함)의 presence_only 와 구분. 토스피드 케이스 | **확정** (2026-06-07, 사용자) |
| MS-D17 | 수집 상한·윈도우 확장 — YouTube 영상 50→100(playlistItems 페이징)·RSS 50→100, 게시 빈도 윈도우 6→12개월. engagement 모집단(최근 100개 영상)과 빈도 모집단(12개월 윈도우)이 다름을 UI 명기 | **확정** (2026-06-07, 사용자) |
